...
export const TaskFilters = memo(function TaskFilters({ search, onSearchChange, filter, onFilterChange, viewMode, onViewModeChange, showForm, onToggleForm }: Props) {
  return (
    <>
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <div className="flex bg-bg-deep border border-border-base rounded-lg p-1">
            <button onClick={() => onViewModeChange('list')} className={cn('p-1.5 rounded-md transition-all', viewMode === 'list' ? 'bg-bg shadow-sm text-brand' : 'text-text-muted hover:text-text-secondary')}><List className="w-4 h-4" /></button>
            <button onClick={() => onViewModeChange('board')} className={cn('p-1.5 rounded-md transition-all', viewMode === 'board' ? 'bg-bg shadow-sm text-brand' : 'text-text-muted hover:text-text-secondary')}><LayoutGrid className="w-4 h-4" /></button>
          </div>
          <Button onClick={onToggleForm}>{showForm ? 'Cancel' : 'Add Task'}</Button>
        </div>
      </div>
